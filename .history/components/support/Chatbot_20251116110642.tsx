...
const Chatbot = () => {
  ...
  const findMatchingTopic = (input: string): keyof typeof knowledgeBase => {
    ...
    for (const [topic, data] of Object.entries(knowledgeBase)) {
      ...
      if (topicData.patterns.some((pattern) => lowerInput.includes(pattern))) {
  ...
  const getQuickRepliesForTopic = (
    topic: keyof typeof knowledgeBase
  ): QuickReply[] => {
    const topicData = knowledgeBase[topic];
